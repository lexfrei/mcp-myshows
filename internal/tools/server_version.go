...
const ServerVersionToolName = "myshows_server_version"
...
func ServerVersionTool() *mcp.Tool {
	return &mcp.Tool{
		...
		Description: "Report the mcp-myshows server version, git revision, and Go runtime version",
...
func NewServerVersionHandler(version, revision, goVersion string) mcp.ToolHandlerFor[ServerVersionParams, ServerVersionResult] {
	return func(
		...
		result := ServerVersionResult{
			...
			Revision:  revision,
			GoVersion: goVersion,
			Output:    fmt.Sprintf("mcp-myshows %s (%s), built with %s", version, revision, goVersion),
		}
